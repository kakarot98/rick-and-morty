...
import { likeCharacter } from "../../../../redux/ducks/characterLike";
...
function CharacterCard({ characterData }) {
  ...
  const handleLike = () => {
    ...
    dispatch(likeCharacter(characterData));
  };
    ...
    <Card sx={{ width: 350, height: 400, background: isFlipped? `linear-gradient(rgba(255,255,255,.5), rgba(255,255,255,.5)), url("${characterData.image}") no-repeat`:'' , backgroundSize:'cover'}} className='card card-container'>
      {!isFlipped ? (
        <>
          ...
          >
            ...
            <CardContent className='card-content-container'>
              ...
              <Typography variant="body1" color="text.primary" className='text text-species'>
          ...
          <CardActions className="card-action btn-container action-container">
            <Button size="small" color="secondary" onClick={() => handleLike()} className='btn btn-like'>
              Like
            </Button>
          </CardActions>
        </>
        ...
        <>
          ...
          <CardActions className="card-action btn-container action-container">
            <Button size="small" onClick={() => handleLike()} className='btn btn-like' variant='contained' color='secondary'>
              Like
            </Button>
          </CardActions>
        </>
